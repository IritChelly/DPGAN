...
class LatentEmbedding(nn.Module):
    ''' projects class embedding onto hypersphere and returns the class embedding '''
...
class LatentConcat(nn.Module):
    ''' returns the concat of the latent (z) and the controid (y) '''

    def __init__(self, *args, **kwargs):
        super().__init__()

    def forward(self, z, y):
        yembed = y / torch.norm(y, p=2, dim=1, keepdim=True)
        yz = torch.cat([z, yembed], dim=1)
        return yz
...
class LinearConditionalMaskLogits(nn.Module):
    ...
    def __init__(self, nc, nlabels, device):
        super().__init__()
        self.fc = nn.Linear(nc, nlabels)
        self.device = device

    def forward(self, inp, y=None, take_best=False, get_features=False):
        out = self.fc(inp)  # batch_sz x nlabels
        if get_features: return out

        if not take_best:
            y = y.view(-1)  # batch_sz x 1
            index = Variable(torch.LongTensor(range(out.size(0))))  # batch_sz x 1
            if y.is_cuda:
                index = index.to(self.device)
            tmp = out[index, y]  # batch_sz x 1
            return tmp
        else:
