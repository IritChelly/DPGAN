...
class Generator(nn.Module):
    def __init__(self,
        ...
        if conditioning == 'embedding':
            ...
            #self.fc = nn.Linear(z_dim + embed_dim, (self.img_sz//8) * (self.img_sz//8) * ngf * 8)
        elif conditioning == 'unconditional':
            self.get_latent = blocks.Identity()
            self.fc1 = nn.Sequential(nn.Linear(z_dim, 1024),
                                    nn.BatchNorm1d(1024),
                                    nn.LeakyReLU(0.2, inplace=True))
            #self.fc = nn.Linear(z_dim, (self.img_sz//8) * (self.img_sz//8) * ngf * 8)
        else:
...
class Discriminator(nn.Module):
    def __init__(self,
        ...
        self.conv1 = nn.Sequential(nn.Conv2d(nc * pack_size * 2, ndf, 3, 1, 1), nn.BatchNorm2d(ndf), nn.LeakyReLU(0.1, inplace=True))
        self.conv2 = nn.Sequential(nn.Conv2d(ndf, ndf, 4, 2, 1), nn.BatchNorm2d(ndf), nn.LeakyReLU(0.1, inplace=True))

        self.conv3 = nn.Sequential(nn.Conv2d(ndf, ndf * 2, 3, 1, 1), nn.BatchNorm2d(ndf * 2), nn.LeakyReLU(0.1, inplace=True))
        self.conv4 = nn.Sequential(nn.Conv2d(ndf * 2, ndf * 2, 4, 2, 1), nn.BatchNorm2d(ndf * 2), nn.LeakyReLU(0.1, inplace=True))

        self.conv5 = nn.Sequential(nn.Conv2d(ndf * 2, ndf * 4, 3, 1, 1), nn.BatchNorm2d(ndf * 4), nn.LeakyReLU(0.1, inplace=True))
        self.conv6 = nn.Sequential(nn.Conv2d(ndf * 4, ndf * 4, 4, 2, 1), nn.BatchNorm2d(ndf * 4), nn.LeakyReLU(0.1, inplace=True))

        self.conv7 = nn.Sequential(nn.Conv2d(ndf * 4, ndf * 8, 3, 1, 1), nn.LeakyReLU(0.1))

        if conditioning == 'mask':
            self.fc_out = blocks.LinearConditionalMaskLogits(int(ndf * 8 * (self.img_sz//8) * (self.img_sz//8)), nlabels, self.device)
        elif conditioning == 'unconditional':
            self.fc_out = blocks.LinearUnconditionalLogits(int(ndf * 8 * (self.img_sz//8) * (self.img_sz//8)))
        else:
            raise NotImplementedError(
    ...
    def forward(self, input_img, y):
        ...
        out = self.conv1(concat_img_and_y)  # (batch_sz, ndf, img_sz, img_sz) = (?, 64, 32, 32)
        out = self.conv2(out)      # (batch_sz, ndf, img_sz//2, img_sz//2)
        out = self.conv3(out)      # (batch_sz, ndf*2, img_sz//2, img_sz//2)
        out = self.conv4(out)      # (batch_sz, ndf*2, img_sz//4, img_sz//4)
        out = self.conv5(out)      # (batch_sz, ndf*4, img_sz//4, img_sz//4)
        out = self.conv6(out)      # (batch_sz, ndf*4, img_sz//8, img_sz//8)
        out = self.conv7(out)      # (batch_sz, ndf*8, img_sz//8, img_sz//8)

        ftrs = out.view(out.size(0), -1)  # shape: (batch_size, ndf * 8 * (img_sz//8) * (img_sz//8))
