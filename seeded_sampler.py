...
class SeededSampler():
    ...
    def get_generator(self):
        ...
        generator, _ = build_models(self.config, self.device)
        #generator = generator.to(self.device)
        #generator = nn.DataParallel(generator)
        generator = nn.DataParallel(generator, device_ids = [int(torch.cuda.current_device()), int(abs(int(torch.cuda.current_device())-1))])
    ...
    def get_yz_dist(self):
        ...
        if self.config['clusterer']['name'] != 'supervised':
            ...
            print('Using DPGMM empirical distribution')
