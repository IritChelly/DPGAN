...
from blocks import LatentEmbeddingConcat, Identity, LinearUnconditionalLogits, LinearConditionalMaskLogits, LatentConcat
...
class G(nn.Module):
    def __init__(self,
        ...
        if conditioning == 'unconditional':
            ...
            self.fc1 = nn.Sequential(nn.Linear(z_dim, act_dim), nn.BatchNorm1d(act_dim),nn.ReLU(True))
        elif conditioning == 'conditional':
            self.embedding = LatentEmbeddingConcat(k_value, embed_size)
            self.fc1 = nn.Sequential(nn.Linear(z_dim + embed_size, act_dim), nn.BatchNorm1d(act_dim), nn.ReLU(True))
        elif conditioning == 'conditioned_by_centroids':
            self.embedding = LatentConcat()
            self.fc1 = nn.Sequential(nn.Linear(z_dim + y_dim, act_dim), nn.BatchNorm1d(act_dim),nn.ReLU(True))
        else:
...
class D(nn.Module):
    ...
    def __init__(self, conditioning, k_value, device, act_dim=200, x_dim=2, y_dim=200):
        super().__init__()
        ...
        if conditioning == 'unconditional':
            ...
            self.fc4 = LinearConditionalMaskLogits(act_dim, k_value, device)
            #self.fc4 = LinearUnconditionalLogits(act_dim)
        elif conditioning == 'conditioned_by_centroids':
